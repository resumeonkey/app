...
async def _adapt_section(
    ...
    # For experience: enforce that non-bullet lines in the LLM output match
    # the original non-bullet lines exactly (prevents shifted-slot formatting bugs).
    if section_name in ("experience", "projects", "volunteer"):
        cleaned = _enforce_header_lines(original_text, cleaned)

    return cleaned


def _enforce_header_lines(original_text: str, adapted_text: str) -> str:
    """
    For experience-like sections: ensure every non-bullet line in the adapted
    output matches the corresponding non-bullet line from the original.

    This prevents the LLM from:
    - Splitting "Company | Role | Dates" into two lines (causes slot shift)
    - Adding "Company, Chile" as an extra line
    - Modifying role title wording

    Non-bullet lines are lines that don't start with "•", "-", or "–".
    Bullet lines are left untouched.
    """
    ...
    # Extract original header lines in order
    orig_headers = [l for l in orig_lines if not _is_bullet(l) and l.strip()]

    # Walk the adapted output, replacing non-bullet lines with the original headers
    header_idx = 0
    result: list[str] = []
    for line in adapted_lines:
        if _is_bullet(line):
            result.append(line)  # bullet — keep LLM's version
        elif line.strip():
            # Non-bullet non-empty line — restore from original
            if header_idx < len(orig_headers):
                result.append(orig_headers[header_idx])
                header_idx += 1
            # If LLM added extra header lines beyond the original, skip them
            # (this removes spurious "Allware, Chile" additions)
        else:
            result.append(line)  # empty line — keep as separator
